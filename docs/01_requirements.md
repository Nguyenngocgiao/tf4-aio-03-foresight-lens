...
> - **W12 (Final Build)**: Team AI sẽ thay thế Skeleton bằng thuật toán AI/Thống kê thực tế. Lúc này, Validation và Logic được siết chặt trong app; còn **IAM Auth (SigV4) được enforce ở tầng edge phía CDO** (Internal ALB / API Gateway `AWS_IAM` + SG-to-SG private subnet), không verify trong code engine — app chỉ đọc `principal_id` đã xác thực để audit. Hợp đồng API (API Contract) giữ nguyên xuyên suốt để CDO không phải sửa code.
