...
> - **W12 (Final Build)**: Team AI sẽ thay thế Skeleton bằng thuật toán AI/Thống kê thực tế. Lúc này, Validation và Logic được siết chặt trong app; còn **IAM Auth (SigV4) được enforce ở tầng edge phía CDO** — **private API Gateway** với `AWS_IAM` authorization đứng trước **Internal ALB** (ALB chỉ route nội bộ; IAM/SigV4 do API Gateway validate), cộng SG-to-SG private subnet — không verify trong code engine, app chỉ đọc `principal_id` đã xác thực để audit. Hợp đồng API (API Contract) giữ nguyên xuyên suốt để CDO không phải sửa code.
...
|---|---|---|---|
| **Lead Time** | ≥ 15 phút trước SLO breach | So sánh thời điểm AI phát ra Alert đầu tiên với thời điểm hệ thống OOM / Throttling trong môi trường test (Chaos Engineering). | SRE cần ít nhất 5 phút để đọc alert, 5 phút để approve, và 5 phút để container mới spin up. |
| **Catch rate (Recall)** | ≥ 0.80 (gate) | TP / (TP + FN) đo trên holdout sliding-window (3 service). | Không bỏ lọt drift dẫn tới SLO breach - đây là KPI chính của Client. |
| **False Positive Rate (FPR)** | ≤ 12% (gate) | FP / (FP + TN) trong điều kiện baseline bình thường + FP trap (noisy baseline). | Hạn chế Alert Fatigue cho đội trực ban (On-call). |
| **Precision (secondary)** | ≥ 0.75 (stretch) | TP / (TP + FP) trên holdout. | Mục tiêu phụ; gate chính là FPR ≤ 12% + Catch ≥ 80%. Window biên vùng anomaly kéo precision xuống ~0.79 (xem 04_eval_report). |
| **Operating Cost** | < $200/tháng | Đo lường bằng AWS Cost Explorer (chỉ tính chi phí cho AI Engine API và lưu trữ Audit Log, không tính hạ tầng của CDO). | Ràng buộc tài chính khắt khe từ ban Giám đốc. Cần giải pháp Lean & Mean. |
...
- **Cross-region failover (DR)**: Thiết kế chỉ dừng ở mức Single-region (us-east-1). Disaster Recovery (DR) sang region khác chỉ nằm trên tài liệu (Design-only).
