...
| **Precision** | ≥ 0.90 (90%) | Tỉ lệ True Positive / (True Positive + False Positive) đo trên 10 test scenarios. | Giảm thiểu tối đa báo động giả, đảm bảo khi Foresight Lens báo động thì 90% là sự thật. |
| **False Positive Rate (FPR)** | < 5% | Đếm số lượng alert bị kích hoạt sai trong điều kiện Traffic bình thường (Normal Baseline). | Hạn chế Alert Fatigue (mệt mỏi vì cảnh báo giả) cho đội ngũ trực ban (On-call engineers). |
