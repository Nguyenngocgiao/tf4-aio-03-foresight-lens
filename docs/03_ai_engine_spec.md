...
> - Yêu cầu: Real engine logic, IAM SigV4 enforced **ở tầng edge phía CDO** — **private API Gateway** (authorizer `AWS_IAM`) validate SigV4, đứng trước **Internal ALB** (ALB chỉ route nội bộ, không tự làm IAM auth), cộng Security Group SG-to-SG trong private subnet — engine chạy sau cổng này nên mọi traffic chạm tới đã được ký & xác thực; app chỉ trích `principal_id` từ header đã verify để ghi audit. Schema API không đổi.
> - Full Docs: TẤT CẢ sections refined với:
> - 5.5 Model NFR Control Matrix có MG-01..MG-08 evidence
> - 6 AI Security thực thi (NOT just spec): IAM SigV4 enforce ở edge qua **private API Gateway (`AWS_IAM`)** + Pydantic input validation (max 10k datapoints, ≥120 phút) + audit log 6 trường + SHA-256 hash PII. KHÔNG dùng Bedrock Guardrails vì engine là statistical (EWMA/STL), không có LLM/prompt để guard.
...
| **Data Poisoning (Bơm nhiễu)** | Attacker giả mạo hệ thống CDO, liên tục gửi dữ liệu rác để làm hỏng Baseline (khiến hệ thống tưởng lượng Traffic ảo là thật). | Medium | **Edge Auth**: Endpoint bảo vệ bằng **API Gateway (`AWS_IAM`) validate SigV4** (no API keys). Bác bỏ mọi request không có chữ ký IAM nội bộ hợp lệ. |
...
| Item | Mô hình chi phí | Chi phí / Tháng |
|---|---|---|
| Compute — ECS Fargate 2×(0.5 vCPU, 1 GB) 24/7, us-east-1 | **Flat** — bill theo task-second, KHÔNG theo request | ~$36 |
| Internal ALB | **Flat** — ~$0.0225/giờ base + LCU | ~$16 |
| Private API Gateway (`AWS_IAM`, validate SigV4) | ~$1/triệu request | ~$0 (mức demo, trong free-tier) |
| S3 (audit KMS + baselines) | Theo dung lượng | ~$1 |
| CloudWatch Logs (5GB) | Free tier | ~$0 |
| **Tổng (flat, request-independent)** | | **~$53 / Tháng** |

> **Kết luận Tài chính**: Chi phí ~**$53/tháng cố định** (KHÔNG per-tenant, KHÔNG per-call — Fargate + ALB là chi phí 24/7 độc lập số request; đây là điểm khác biệt cốt lõi so với LLM tính token), nằm sâu dưới Budget $200. Inference token cost = $0 (statistical engine, no LLM). Khớp `tf4-evidence/evidence/evidence_cost.json` và `04_eval_report.md §6`.
...
- **Network (Mạng nội bộ)**: Các container Fargate được nhốt hoàn toàn trong dải mạng **Private Subnet**. CDO gọi AI API qua **private API Gateway (`AWS_IAM` → validate SigV4)** rồi tới **Internal Application Load Balancer (ALB)** route vào Fargate (ALB internal-only). AI Engine không hề có kết nối mở ra Internet công cộng (No Public IP).
